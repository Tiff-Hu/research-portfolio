...
library(baguette) library(rpart)
...
+
  ...
  theme(text = element_text(size = 11), plot.title = element_text(size =
        ...
        axis.title.y = element_text(hjust = 0.5)) + scale_y_discrete(name = "Racial Group",
                                                                     labels = c("One race", "Two or more races", "White", "Black or African American", Indian and Alaska Native", "Asian", "Other Asian")) +
guides(fill = "none")
"American
...
cols_to_drop_1 = c("x0001e", "x0018e","x0019e", "x0020e", "x0021e", "x0022e", "x0023e","x0024e" , "x0025e", "x0026e", "x0027e",
                   ...
                   "x0002e","x0003e","x0005e","x0006e","x0007e","x0008e","x0009e","x0010e","x0011e","x 0012e","x0013e","x0014e","x0015e", "x0016e",
                   "x0017e")
...
code2013 <- c("one", "two", "three", "four", "five", "six") custom_recipe <-
  ...
  step_mutate(pct_55to64 = (x0013e + x0014e) / x0001e) %>% step_mutate(pct_65andovr = (x0015e +
...
%>%
  ...
  step_mutate(pc_associate_25over = c01_011e / c01_006e ) %>% step_mutate(pc_bachlor_25over = c01_012e /
...
# Use workflow_map to call in v-fold cross validation data lm_models lm_models %>% workflow_map("fit_resamples",
# Options to `workflow_map()`: seed = 1,
verbose = TRUE,
# Options to `fit_resamples()`:
resamples = train_folds, control = keep_pred)
<-
  # This shows that the custom recipe performs best lm_models collect_metrics() %>%
  filter(.metric == "rmse")
%>%
  autoplot(lm_models)
...
<-
  ...
  autoplot(svm_res)
  # Boosted trees - xgboost fit bt_xgboost_workflow <- workflow() %>%
  ...
  # Boosted trees - lightgbm fit bt_light_workflow <- workflow() %>%
  ...
  # Stacked model
  # Define stack object adding the top four candidate models test_stack stacks() %>% add_candidates(knn_res) %>%
